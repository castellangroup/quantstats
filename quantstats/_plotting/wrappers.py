...
def snapshot(
    ...
    **kwargs,
    ...
    returns = _utils.make_portfolio(returns.dropna(), 1,
                                    mode).pct_change().fillna(0)
    ...
    if not grayscale:
        if isinstance(dd, _pd.Series):
            ...
            for i, col in enumerate(dd.columns):
                axes[1].fill_between(dd[col].index,
                                     0,
                                     dd[col],
                                     color=colors[i + 1],
                                     alpha=0.25)
    ...
    if subtitle:
        ax.set_title(
            "\n%s - %s ;  P&L: %s (%s)                " % (
                ...
                _utils._score_str("{:,}%".format(
                    round((returns.values[-1] / returns.values[0] - 1) * 100,
                          2))),
            ),
            fontsize=12,
            color="gray",
...
def rolling_volatility(
    ...
    if benchmark is not None:
        benchmark = _utils._prepare_benchmark(benchmark, returns.index)
        benchmark = _stats.rolling_volatility(benchmark,
                                              period,
                                              periods_per_year,
...
def rolling_sharpe(
    ...
    if benchmark is not None:
        ...
        benchmark = _stats.rolling_sharpe(benchmark,
                                          rf,
                                          period,
                                          True,
                                          periods_per_year,
...
def rolling_sortino(
    ...
    returns = _stats.rolling_sortino(returns, rf, period, True,
                                     periods_per_year)

    if benchmark is not None:
        benchmark = _utils._prepare_benchmark(benchmark, returns.index, rf)
        benchmark = _stats.rolling_sortino(benchmark,
                                           rf,
                                           period,
                                           True,
                                           periods_per_year,
